...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    softmax_loss = np.sum(-scores[range(N), y] + np.log(np.sum(np.exp(scores), axis = 1)) )
    sum_norm = np.sum(W1**2) + np.sum(W2**2)
    l2_reg = 0.5*reg*sum_norm
    ...
    soft_max = exp_score / np.sum(exp_score, axis = 1, keepdims = True)
    soft_max[range(N), y] -= 1
    dl_da = soft_max/N
    
    grads['W2']= dl_da.T @ f_h1 + reg * W2
    grads['b2'] = np.sum(dl_da, axis=0, keepdims=True)
    da_dh1 = W2.T
    dl_dz1 = (W1.dot(X.T) > 0) * da_dh1.dot(dl_da.T)
    grads['W1'] = dl_dz1.dot(X) + reg * W1
    grads['b1'] = np.sum(dl_dz1, axis=1, keepdims=True).T
    # ================================================================ #
    # END YOUR CODE HERE
    # ================================================================ #
  ...
  def predict(self, X):
    ...
    scores = relu_h1 @ self.params['W2'].T + self.params['b2']
